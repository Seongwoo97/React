...
// styled 가 적용된 컴포넌트를 만들어주는 객체 import
...
class Home extends Component{
    render(){
        return(
            <div>
                {/* 1) 인라인 스타일 : html요소에 style 속성으로 스타일을 지정 [다른점 : 스타일을 문자열이 아니라 JS객체로 줘야 함!] */}
                ...
                {/* 3) 클래스 선택자를 특정 요소에만 적용되도록 하는 module.css 파일 [인기 없음] */}
                <p className="bb">모듈 클래스 선택자 연습</p>
                {/* 위 방식으로는 안됨. '모듈이 가지고 있는 bb 라고 지정해야 함!' */}
                <p className={homeStyle.bb}>모듈 클래스 선택자 실습</p>


                {/* 4) 외부 스타일 라이브러리 - sass/scss [라이브러리 설치 필요 npm install sass (scss 모듈이 포함되어 있음)]*/}
                ...
                {/* styled-components 를 사용하려면 import 해야 함 */}
...
const textStyle= {
    color: 'red',
    fontSize: 20, // snake 표기법이 cammel 표기법으로 바뀜. [기본단위 px] 
    fontStyle: 'italic',
    margin: '16px', // 단위를 사용하려면 문자열로 줘야 함
}


// styled-components 를 이용하여 스타일링이 된 버튼 컴포넌트 만들어놓기
// 스타일을 작성하는 방법은 빽틱 영역 사용 [css문법으로.] -- vscode에서 속성명이 자동완성되려
...
/* 스타일이 된 버튼에 지정된 속성(property)를 받아서 특정 기능으로 처리할 수 있음. */
