...
// src 폴더 안에 있는 이미지파일을 불러와서 변수에 저장
    ...
    render(){
        return (
            <div>
                ...
                {/* (경로1) html에서 처럼 파일경로를 지정하면 안 보임. */}
                <img src="./penguins.jpg" alt="penguins" />
                {/* (해결) src 폴더의 이미지를 .jsx 파일에서 불러와서 보여줘야 함 import */}
                <img src={penguins} alt="penguins" width={150} />

                {/* 2) src 폴더 안에 이미지파일을 위치하면 파일이 너무 많이 식별이 어려움.  */}
                {/* ★ 그래서 src/assets 폴더 안에 이미지를 위치할 것을 권장 ★ */}
                <img src={paris} alt="paris image" width={150} />

                {/* 3) public 폴더는 index.html이 바라보는 본인 폴더 위치임. */}
                {/* public 폴더에 이미지파일을 위치하고 사용해보기 */}
                <img src="./koala.jpg" alt="koala image" width={150} />

                {/* 4) 웹 이미지파일의 전체 url 경로 (절대경로) */}
                <img src="https://cdn.pixabay.com/photo/2019/09/17/02/20/jeju-4482313_1280.jpg" alt="web image" width={150} />


                {/* 이미지파일을 사용하는 방법은 경로에 따라 다름. 이 중 권장은 src/assets 폴더 */}
                {/* public 폴더 안에는 아이콘 이미지들을 보통 배치하여 사용. 일반적으로 쓰는 '이미지' 들은 assets 으로 */}
                <hr />
