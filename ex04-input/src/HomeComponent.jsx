...
class HomeComponent extends Component{
    ...
    // 값이 변경되면 자동으로 화면을 갱신해주는 Component 클래스의 아주 특별한 멤버변수 state
    state= {
        text : '',
        text2: '',
    ...
    render(){
        return (
            ...
            <div>
                ...
                {/* 1. input 요소의 글씨가 변경될 때 마다 반응하기 */}
                <input type="text" onChange={this.changeText} />

                <hr />

                {/* 2. input 요소의 글씨가 변경될때마다 바로 아래 p요소에 표시하기 */}
                ...
                {/* 사용자 입력데이터는 보통 웹서버로 전송해야 함. form에 대한 수업(추후 별도로..) */}
    ...
    // 2) 글씨가 변경될때마다 p요소가 보여주는 state값 변경하는 함수 만들기
    changeText2 = (event) => {
        // state 값의 변경은 반드시 .setState()라는 기능을 사용해야만 함
        this.setState({text:event.target.value})
    }


    // 1) 글씨가 변경될때마다 반응할 함수로 등록된 함수 만들기
    changeText = (event) => { // event : 변경 이벤트 정보를 가진 객체가 자동으로 전달됨
        // alert(event.target.value) // 이벤트를 발생한 녀석(target)의 값 출력하기
