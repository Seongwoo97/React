...
// 리액트에서 화면의 구성요소가 되려면.. component 라는 클래스를 상속해야 함.
class MyApp extends Component{ // 상속을 통해 Component의 능력 (멤버들)을 가지게 됨.
    ...
    render(){
        ...
        // 따옴표 없이 태그문<>을 사용하면 React가 알아서 DOM 객체로 만들어 화면을 구성함
        // return <h2>Hello Webapp</h2>
        // 이런 식으로 .js 안에 html태그문을 같이 사용할 수 있도록 만든 언어 : JSX언어 (JS + xml)

        // 요소 여러개를 표시해보기 -- return은 하나의 요소만 가능함
        // return <h2>Hello</h2>
        // <p>error...</p>

        // 그래서 여러 요소를 감싸는 큰 요소 하나를 먼저 만들고 안에 여러개의 요소를 배치
        // JSX 언어의 특징은 js 안에 <> 태그문을 사용하듯이..
        // <> 태그 안에서 js의 변수 사용이나 함수 호출을 수행할 수도 있음.
        let name ='sam'
        ...
        return (
            // class명 지정할 때 속성명을 class 대신 className 사용을 권장 - why? JS에서 class는 설계도를 만드는 키워드
            <div className='wrap'>
                ...
                <p>Hello name</p>
                <p>Hello {name}</p>
                {/* xml 영역 안에서 js를 사용하는 {}를 쓰고 JS의 주석문법으로.. 주석을 구현 */}
                {/* ctrl + / 을 누르면 알아서 주석구조가 만들어짐 */}

                <input type="text"/>
                {/* 속성값도 JS변수로 설정 가능! */}
                <input type= {type} />

                {/* JSX언어는 반드시 end tag 가 있어야 함. 만약 없애려면.. closing tag 사용 */}
                <hr />

                {/* 이미지 보여주기 (별도 수업예정) - 맛보기 */}
                {/* src 폴더에서 작업하는 component의 이미지들은 src 폴더 안에 위치하는 것을 선호함 */}
                {/* 이미지를 사용하려면 import 해야 사용 가능함. */}
                <img src={koala} alt="koala img" />
